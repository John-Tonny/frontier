...
OFFCHAINWORKER="--offchain-worker=always"
...
run_program ()
{
  ...
  nohup $CMD --chain=$CHAIN --base-path=$MYPATH $OFFCHAINWORKER $PRUNING $EXECUTIION $VALIDATOR --node-key=$NODEKEY --port=$PORT --ws-port=$WSPORT --rpc-port=$RPCPORT $RPCCORS $WSEXTERNAL $RPCEXTERNAL $RPCMETHODS --sync=$SYNCMODE >$LOGFILE 2>&1 &
